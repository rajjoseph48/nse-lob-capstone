...
one-month NSE sample. Multi-seed confidence intervals (Tier~C) are needed before claiming significance.
...
\noindent\emph{(C) Rigour layer (expected practice).} Hyperparameter optimization (Optuna) on the best
variant with an equal budget for baselines; a class-imbalance study (focal loss vs class-weighted CE vs
class-balanced sampling vs label smoothing); statistical significance (multi-seed bootstrap CIs +
Diebold--Mariano / permutation tests for pairwise comparison); and probability calibration (temperature
scaling) feeding the cost-aware backtest.
